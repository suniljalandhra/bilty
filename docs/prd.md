...
The complete V1 includes Google OAuth, self-service company onboarding, invitations, company settings, parties, bilty lifecycle, branded A4/80mm output and WhatsApp sharing. The implemented backend delivers Google OAuth, rotating sessions, self-service company setup, invitations, company/member/party management and authenticated bilty HTTP endpoints backed by PostgreSQL. The local V1 also implements the Next.js frontend, A4/80mm PDF generation and expiring/revocable PDF sharing. Live Google consent requires operator credentials; public deployment is not included. Company logos can be embedded from bounded PNG/JPEG uploads; document attachment uploads and assisted onboarding remain deferred. See README for executable commands and verification.
...
- Consignor and consignee each carry a saved-party reference and/or manually entered snapshot. Validate every referenced party belongs to the company and matches its role. Keep names, addresses, GSTINs and phones in the document snapshot. Address-book deletion archives the party: old references remain valid, while new selections must be active.
...
The HTTP boundary implements verified Google identity, access/refresh handling, rotated and revocable refresh sessions, membership revocation, DTO validation and a global Nest authentication guard. Transactional services enforce tenant and administrator permissions. Invitations must use cryptographically random tokens stored hashed, expiry, single use, revocation and an exact normalized invited-email match. Company roles do not grant platform-admin privileges. One company per user is enforced by membership user ID; revoked users cannot self-onboard into another company. Assisted onboarding is deferred and will need a separate trusted administrative path. See docs/auth-api.md for the implemented protocol.
...
WhatsApp handoff opens a user-controlled compose window containing text and an expiring PDF link. Each hashed link stores a fixed issued version, expires within seven days, can be revoked and becomes unavailable if its bilty is cancelled. Edits do not silently change previously shared snapshots; generate a new link to share an updated copy. No message is sent automatically.
